...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const start = async () => {
    ...
    mindarThree = new window.MINDAR.IMAGE.MindARThree({
      ...
      imageTargetSrc: 'assets/targets/witch.mind' // Define the image target file
    ...
    const solution = await loadGLTF('./assets/models/wiz1/wizard1/scene.gltf');
    
    solution.scene.scale.set(1.5, 1.5, 1.0);  // Scale the model
    solution.scene.position.set(0, -0.2, 0);  // Set the model's position in the scene

    // Load the bear 3D model
    const treasure = await loadGLTF('./assets/models/wiz2/wizard2/scene.gltf');
    treasure.scene.scale.set(1.7, 1.7, 1.7);
    treasure.scene.position.set(0, -0.2, 0);
    ...
    raccoonAnchor.group.add(solution.scene);

    // Load and add audio to the raccoon target
    const audioClip1 = await loadAudio('assets/sounds/marker1.mp3');
    ...
    bearAnchor.group.add(treasure.scene);

    const audioClip2 = await loadAudio('./assets/sounds/marker2.mp3');
